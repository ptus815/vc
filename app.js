(function () {
    ...
    setTimeout(() => {
        ...
        class Particle {
            ...
            draw(isWhiteTheme) {
                ctx.fillStyle = isWhiteTheme ? 'rgba(0, 112, 243, 0.4)' : 'rgba(50, 145, 255, 0.5)';
                ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); ctx.fill();
            }
        }

        function initParticles() {
            particles = []; const numParticles = Math.floor((width * height) / 3000); 
        ...
        function animateParticles() {
            ...
            const lineColor = isWhiteTheme ? '0, 112, 243' : '50, 145, 255';
            for (let i = 0; i < particles.length; i++) {
                particles[i].update(); particles[i].draw(isWhiteTheme);
                const dx = mouse.x - particles[i].x; const dy = mouse.y - particles[i].y;
                const dist = Math.sqrt(dx * dx + dy * dy);
                if (dist < 140) { 
                    ctx.beginPath();
                    ctx.strokeStyle = `rgba(${lineColor}, ${0.4 * (1 - dist / 140)})`; 
        ...
        const customSelectTriggerText = customSelectTrigger.querySelector('span');
        const customOptions = document.querySelectorAll('.custom-option');
        const hiddenSubDomain = document.getElementById('subDomainSelect');
        ...
        customOptions.forEach(option => {
            option.addEventListener('click', () => {
                ...
                customSelectTriggerText.textContent = value;
                hiddenSubDomain.value = value;
        ...
        const promptText = "点击获取订阅链接，\n或直接粘贴 vless://、trojan://、vmess:// 等链接点击生成订阅...\n支持批量，每行一个";
        let charIndex = 0;

        function typeWriterEffect() {
            if (charIndex < promptText.length) {
                urlInput.setAttribute('placeholder', promptText.substring(0, charIndex + 1) + '_');
                charIndex++; setTimeout(typeWriterEffect, Math.random() * 40 + 30);
            } else {
                let blinkCount = 0;
                const blinkInterval = setInterval(() => {
                    blinkCount++; urlInput.setAttribute('placeholder', (blinkCount % 2 === 0) ? promptText + '_' : promptText);
                    if (blinkCount >= 6) { clearInterval(blinkInterval); urlInput.setAttribute('placeholder', promptText); }
                }, 400);
            }
        }
        setTimeout(typeWriterEffect, 600);

        let currentFinalSubUrl = '';
        ...
        function extractDefaultValue(code, varName) {
            ...
            let expr = match[1].trim(); const orIndex = expr.lastIndexOf('||');
            if (orIndex !== -1) expr = expr.substring(orIndex + 2).trim();
            expr = expr.replace(/;.*$/, '').trim();
            const strMatch = expr.match(/^(['"])(.*)\1$/); if (strMatch) return strMatch[2];
            try { const result = eval(expr); if (typeof result === 'string') return result; } catch (e) {} return null;
        }
        function parseConfig(code) {
            try {
                let decoded = unescapeUnicode(code); decoded = resolveReverseStrings(decoded);
                const domain = extractDefaultValue(decoded, 'DOMAIN'); const subPath = extractDefaultValue(decoded, 'SUB_PATH');
                if (domain && subPath) return { domain, subPath }; return null;
            } catch (e) { return null; }
        }
        function buildUrl(domain, subPath) {
            domain = domain.replace(/^https?:\/\//, ''); subPath = subPath.replace(/^\/+/, ''); return `https://${domain}/${subPath}`;
        }
        function decodeBase64Safe(str) {
            try { return decodeURIComponent(escape(atob(str))); } catch(e) { try { return atob(str); } catch(e2) { return str; } }
        }

        function getSelectedSubDomain() { return document.getElementById('subDomainSelect').value; }
        ...
        function copyToClipboard(text) {
            ...
            try { document.execCommand('copy'); } catch (err) {} document.body.removeChild(ta);
        }
        function processDirectNode(link) {
            const vlessMatch = link.match(/vless:\/\/([a-fA-F0-9\-]+)@[^?]+\?([^#\n\r]+)/);
            if (vlessMatch) { const uuid = vlessMatch[1]; const params = vlessMatch[2]; const subDomain = getSelectedSubDomain(); return { success: true, url: `https://${subDomain}/sub?uuid=${uuid}&${params}` }; }
        ...
        function renderAndAnimate(results) {
            ...
            for (const item of results) {
                if (item.success) {
                    ...
                    html += `<div class="result-item"><div class="icon-box"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div><div class="info-group"><a href="${item.url}" target="_blank" class="url">${item.url}</a><div class="auto-copy-tag"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg> 复制成功</div></div></div>`;
                } else {
                    html += `<div class="result-item fail"><div class="icon-box"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></div><div class="info-group"><span class="error-msg">${item.error}</span></div></div>`;
                }
            }
        ...
        function getProxyUrl(url) { return 'https://corsproxy.io/?' + encodeURIComponent(url); }

        async function processAndFetchDeep(urlStr) {
            try {
                const configResponse = await fetch(getProxyUrl(urlStr));
                if (!configResponse.ok) throw new Error();
                const rawText = await configResponse.text(); const decodedText = decodeBase64Safe(rawText);
                let res = processDirectNode(decodedText); if (res.success) return res;
                res = processDirectNode(rawText); if (res.success) return res;
                return { success: false, error: 'ERR' };
            } catch (err) { return { success: false, error: 'ERR' }; }
        }

        async function executeRemoteFetch(urls) {
            fetchBtn.disabled = true; fetchBtn.textContent = '...';
            const results = [];
            for (const url of urls) {
                if (/^(vless|trojan|ss|vmess):\/\//i.test(url)) { results.push(processDirectNode(url)); continue; }
                try {
                    const response = await fetch(getProxyUrl(url));
                    if (!response.ok) throw new Error();
                    const code = await response.text(); const config = parseConfig(code);
                    if (config) {
                        const targetUrl = buildUrl(config.domain, config.subPath);
                        const deepResult = await processAndFetchDeep(targetUrl); results.push(deepResult);
                    } else { results.push({ success: false, error: 'ERR' }); }
                } catch (err) { results.push({ success: false, error: 'ERR' }); }
            }
            renderAndAnimate(results); fetchBtn.disabled = false; fetchBtn.textContent = '生成订阅';
        }
        ...
        urlInput.addEventListener('keydown', (e) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); fetchBtn.click(); }
        });

        document.getElementById('themeToggle').addEventListener('click', () => { document.body.classList.toggle('white-theme'); });

        document.getElementById('subConverterBtn').addEventListener('click', () => {
        ...
        getRandomLinkBtn.addEventListener('click', async () => {
            try {
                getRandomLinkBtn.disabled = true;
                getRandomLinkBtn.textContent = '...';
                const res = await fetch(getProxyUrl('https://seep.eu.org/https://raw.githubusercontent.com/ptus815/vc/refs/heads/main/vc.txt'));
                ...
                getRandomLinkBtn.disabled = false;
                getRandomLinkBtn.textContent = '获取订阅链接';
            }
        });

        const v2rayNGIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/v2rayNG.svg" alt="v2rayNG">`;
        const clashIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/ClashMetaforAndroid.svg" alt="Clash Meta">`;
        const clashVergeIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/ClashVergeRev.svg" alt="Clash Verge">`;
        const flClashIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/Flclash.svg" alt="FlClash">`;
        const surfboardIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/Surfboard.svg" alt="Surfboard">`;
        const singboxIcon = `<img class="app-icon" src="https://raw.githubusercontent.com/ptus815/vc/main/icon/singbox.svg" alt="Sing-box">`;
        ...
        const dlData = {
            android: [
                { name: 'FlClash', icon: flClashIcon, links: [ { name: 'Android Universal APK', repo: 'chen08209/FlClash', match: 'android-arm64-v8a.apk' } ] },
                { name: 'v2rayNG', icon: v2rayNGIcon, links: [ { name: 'Universal APK', repo: '2dust/v2rayNG', match: 'universal.apk' }, { name: 'Arm64 APK', repo: '2dust/v2rayNG', match: 'arm64-v8a.apk' } ] },
                { name: 'Clash Meta for Android', icon: clashIcon, links: [ { name: 'Release APK', repo: 'MetaCubeX/ClashMetaForAndroid', match: 'meta-universal-release.apk' } ] },
                { name: 'Surfboard', icon: surfboardIcon, links: [ { name: 'Universal APK', repo: 'getsurfboard/surfboard', match: 'universal-release.apk' } ] },
                { name: 'Sing-box', icon: singboxIcon, links: [ { name: 'Release', url: 'https://github.com/SagerNet/sing-box/releases/latest' } ] },
                { name: 'Hiddify', icon: hiddifyIcon, links: [ { name: 'Android Universal APK', repo: 'hiddify/hiddify-next', match: 'Android-universal.apk' } ] },
                { name: 'NekoBoxForAndroid', icon: nekoIcon, links: [ { name: 'Arm64 APK', repo: 'MatsuriDayo/NekoBoxForAndroid', match: 'arm64-v8a.apk' } ] },
                { name: 'Karing', icon: karingIcon, links: [ { name: 'Android Arm64 APK', repo: 'KaringX/karing', match: 'android_arm64-v8a.apk' } ] }
            ],
            windows: [
                { name: 'Clash Verge Rev', icon: clashVergeIcon, links: [ { name: 'Windows Setup (x64)', repo: 'clash-verge-rev/clash-verge-rev', match: 'x64-setup.exe' }, { name: 'Windows Portable', repo: 'clash-verge-rev/clash-verge-rev', match: 'x64.app.tar.gz' } ] },
                { name: 'FlClash', icon: flClashIcon, links: [ { name: 'Windows Setup (x64)', repo: 'chen08209/FlClash', match: 'windows-amd64-setup.exe' } ] },
                { name: 'v2rayN', icon: v2rayNGIcon, links: [ { name: 'Windows Desktop (Zip)', repo: '2dust/v2rayN', match: 'windows-64-desktop.zip' } ] },
                { name: 'Sing-box', icon: singboxIcon, links: [ { name: 'Windows (amd64)', repo: 'SagerNet/sing-box', match: 'windows-amd64.zip' } ] },
                { name: 'Hiddify', icon: hiddifyIcon, links: [ { name: 'Windows Setup', repo: 'hiddify/hiddify-next', match: 'Windows-Setup-x64.exe' } ] },
                { name: 'NekoRay', icon: nekoIcon, links: [ { name: 'Windows (Zip)', repo: 'MatsuriDayo/nekoray', match: 'windows64.zip' } ] },
                { name: 'Karing', icon: karingIcon, links: [ { name: 'Windows Setup', repo: 'KaringX/karing', match: 'windows_x64.exe' } ] }
            ],
            mac: [
                { name: 'Clash Verge Rev', icon: clashVergeIcon, links: [ { name: 'macOS Apple Silicon (DMG)', repo: 'clash-verge-rev/clash-verge-rev', match: 'aarch64.dmg' }, { name: 'macOS Intel (DMG)', repo: 'clash-verge-rev/clash-verge-rev', match: 'x64.dmg' } ] },
                { name: 'FlClash', icon: flClashIcon, links: [ { name: 'macOS Apple Silicon (DMG)', repo: 'chen08209/FlClash', match: 'macos-arm64.dmg' } ] },
                { name: 'Sing-box', icon: singboxIcon, links: [ { name: 'Release', url: 'https://github.com/SagerNet/sing-box/releases/latest' } ] },
                { name: 'Hiddify', icon: hiddifyIcon, links: [ { name: 'macOS DMG', repo: 'hiddify/hiddify-next', match: 'MacOS.dmg' } ] },
                { name: 'NekoRay', icon: nekoIcon, links: [ { name: 'Release', url: 'https://github.com/MatsuriDayo/nekoray/releases/latest' } ] },
            ...
            linux: [
                ...
                { name: 'FlClash', icon: flClashIcon, links: [ { name: 'Linux AppImage', repo: 'chen08209/FlClash', match: 'linux-amd64.AppImage' } ] },
                { name: 'Sing-box', icon: singboxIcon, links: [ { name: 'Linux amd64 Deb', repo: 'SagerNet/sing-box', match: 'linux_amd64.deb' } ] },
                { name: 'Hiddify', icon: hiddifyIcon, links: [ { name: 'Linux AppImage', repo: 'hiddify/hiddify-next', match: 'Linux-x64-AppImage.AppImage' } ] },
                { name: 'NekoRay', icon: nekoIcon, links: [ { name: 'Linux AppImage', repo: 'MatsuriDayo/nekoray', match: 'linux-x64.AppImage' } ] },
                { name: 'Karing', icon: karingIcon, links: [ { name: 'Linux amd64 Deb', repo: 'KaringX/karing', match: 'linux_amd64.deb' } ] }
            ]
        };
        ...
        function renderOS(os) {
            dlGrid.innerHTML = '';
            dlData[os].forEach(app => {
                const wrap = document.createElement('div');
                wrap.className = 'dl-item';
                const linksHTML = app.links.map(l => {
                    if (l.url) {
                        return `<a class="dl-link" href="${l.url}" target="_blank"><svg class="dl-link-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>${l.name}</a>`;
                    } else if (l.repo) {
                        return `<a class="dl-link dynamic-dl" href="#" data-repo="${l.repo}" data-match="${l.match}"><svg class="dl-link-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg><span class="dl-text">${l.name}</span></a>`;
                    }
                }).join('');
                wrap.innerHTML = `<button class="app-btn dl-header">${app.icon}<span class="app-name">${app.name}</span><svg class="dl-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg></button><div class="dl-links">${linksHTML}</div>`;
                dlGrid.appendChild(wrap);
                wrap.querySelector('.dl-header').addEventListener('click', () => {
                    const ls = wrap.querySelector('.dl-links');
                    const isOpen = ls.classList.contains('open');
                    document.querySelectorAll('.dl-links').forEach(e => e.classList.remove('open'));
                    document.querySelectorAll('.dl-chevron').forEach(e => e.style.transform = 'rotate(0deg)');
                    if (!isOpen) {
                        ls.classList.add('open');
                        wrap.querySelector('.dl-chevron').style.transform = 'rotate(180deg)';
                    }
                });
            });
        }
        ...
        document.addEventListener('click', async (e) => {
            const dlLink = e.target.closest('.dynamic-dl');
            if (!dlLink) return;
            e.preventDefault();
            if (dlLink.classList.contains('loading')) return;
            ...
            try {
                const repo = dlLink.dataset.repo;
                const matchStr = dlLink.dataset.match;
                ...
                window.open(`https://github.com/${dlLink.dataset.repo}/releases/latest`, '_blank');
                textSpan.innerText = originalText;
                dlLink.classList.remove('loading');
            }
        });
